...
class App(wx.Frame):
    ...
    def __init__(self):
        ...
        self.languageOptions = ['', 'English', 'German', 'Slovak']
        self.selectedLanguage = ''
        self.shuffleOptions = ['', 'Sentences', 'Words']
        self.selectedShuffle = ''
        self.shuffleSeed = 42
        self.udpipeModels = {}
        self.udpipePipelines = {}
        self.modelPaths = {
            'English': self.resolveModelPath('english-ewt-ud-2.5-191206.udpipe'),
            'German': self.resolveModelPath('german-gsd-ud-2.5-191206.udpipe'),
            'Slovak': self.resolveModelPath('slovak-snk-ud-2.5-191206.udpipe'),
        }
    ...
    # Resolve UDPipe model path relative to this file or the shared model folder
    def resolveModelPath(self, modelName):
        appDir = Path(__file__).resolve().parent
        candidates = [
            appDir / modelName,
            appDir.parent / 'UDPipeModels' / modelName,
        ]
    ...
    def initGUI(self):
        ...
        self.shuffleSeedInput = wx.TextCtrl(panel, value=str(self.shuffleSeed), size=(70, -1))
        self.shuffleSeedInput.Bind(wx.EVT_TEXT, self.shuffleSeedChange)
        ...
        additionalOptionsLayout.Add(wx.StaticText(panel, label='Seed:'), flag=wx.LEFT | wx.ALIGN_CENTER_VERTICAL, border=20)
        additionalOptionsLayout.Add(self.shuffleSeedInput, flag=wx.LEFT, border=10)
    ...
    # Shuffle seed selection event handler
    def shuffleSeedChange(self, event):
        seedValue = self.shuffleSeedInput.GetValue().strip()
        if seedValue:
            try:
                self.shuffleSeed = int(seedValue)
            except ValueError:
                pass
    ...
    # Load and cache UDPipe pipeline for selected language
    def getUdpipePipeline(self):
        if self.selectedLanguage not in self.modelPaths:
            raise ValueError(f'Unsupported language: {self.selectedLanguage}')

        if self.selectedLanguage not in self.udpipePipelines:
            modelPath = self.modelPaths[self.selectedLanguage]
            model = Model.load(str(modelPath))
            if not model:
                raise FileNotFoundError(f'UDPipe model could not be loaded: {modelPath}')
            self.udpipeModels[self.selectedLanguage] = model
            self.udpipePipelines[self.selectedLanguage] = Pipeline(
                model,
                'tokenize',
                Pipeline.DEFAULT,
                Pipeline.DEFAULT,
        ...
        return self.udpipePipelines[self.selectedLanguage]


    # Parse text with UDPipe and return sentences made of token metadata
    def parseTextWithUdpipe(self, text):
        pipeline = self.getUdpipePipeline()
        ...
        for line in processed.splitlines():
            ...
            lemma = cols[2]
            upos = cols[3]
            normalizedToken = self.normalizePunctuation(token)
            normalizedLemma = self.normalizePunctuation(lemma if lemma != '_' else token)
            lemmaIsPunctuation = self.isKnownPunctuation(normalizedLemma)
            isPunctuation = upos == 'PUNCT' or self.isKnownPunctuation(normalizedToken) or lemmaIsPunctuation

            currentSentence.append({
                'token': normalizedLemma if lemmaIsPunctuation else normalizedToken,
    ...
    # Normalize quote/apostrophe/ellipsis variants to one graph node
    def normalizePunctuation(self, token):
        return self.punctuationNormalization.get(token, token)


    # Check if a token belongs to any supported punctuation group
    def isKnownPunctuation(self, token):
        return any(token in values for values in self.punctuationTokens.values())


    # Convert UDPipe token metadata to the tokens selected for analysis
    def filterUdpipeTokens(self, sentences, selectedPunctuation=None):
        selectedPunctuation = selectedPunctuation or {}
        selectedPunctuationTokens = set()
        for key in selectedPunctuation.keys():
            selectedPunctuationTokens.update(self.punctuationTokens.get(key, set()))
        selectedPunctuationTokens = {
            self.normalizePunctuation(token) for token in selectedPunctuationTokens
        }
        ...
        for sentence in sentences:
            for item in sentence:
                if item['isPunctuation']:
                    if selectedPunctuationTokens and item['token'] in selectedPunctuationTokens:
                        tokens.append(item['token'])
                else:
                    tokens.append(item['lemma'].lower())

        return tokens


    # Return a deterministic random generator for repeatable shuffle analysis
    def getShuffleRandom(self):
        return random.Random(self.shuffleSeed)


    # Shuffle full UDPipe-detected sentences before flattening tokens
    def applySentenceShuffle(self, sentences):
        shuffledSentences = list(sentences)
        if self.selectedShuffle == 'Sentences':
            self.getShuffleRandom().shuffle(shuffledSentences)
        return shuffledSentences


    # Tokenize text file with UDPipe and return a list of lemmas/punctuation
    def processTextFile(self, selectedPunctuation=None):
        text = self.readTextFile(self.inputTextFile)
        sentences = self.parseTextWithUdpipe(text)
        sentences = self.applySentenceShuffle(sentences)
        data = self.filterUdpipeTokens(sentences, selectedPunctuation)
        if self.selectedShuffle == 'Words':
            self.getShuffleRandom().shuffle(data)
        return data
    ...
    def createGraphData(self, data):
        ...
        previousWord = None
        for element in data:
            element = self.normalizePunctuation(element)
    ...
    def calculateValues(self, graphData, occurrenceData):
        ...
        punctuationCounts = Counter([char for char in data if self.isKnownPunctuation(char)])
